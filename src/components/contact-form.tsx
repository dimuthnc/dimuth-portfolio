    ...
    <form onSubmit={onSubmit} noValidate className="space-y-5">
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          {errors.name && (
            <p id={nameId + "-err"} className="site-error">{errors.name}</p>
          )}
        </div>
        <div>
          ...
          {errors.email && (
            <p id={emailId + "-err"} className="site-error">{errors.email}</p>
          )}
        </div>
      </div>
      <div>
        ...
        {errors.message && (
          <p id={messageId + "-err"} className="site-error">{errors.message}</p>
        )}
      </div>
      ...
      <div className="pt-2">
        <Button type="submit" disabled={!canSubmit} aria-disabled={!canSubmit}>
          {pending ? "Sending…" : "Send message"} <Send aria-hidden />
        </Button>
      </div>
    </form>
  )
